...
void _RumbleTimer(struct timer_list *t)
{
    extern void MIYOO_RUMBLE(bool);
    MIYOO_RUMBLE(false);
}

void _FlashTimer(struct timer_list *t)
{
    ...
    if (flash_count < 10) {
        ...
        flash_count += 1;
        timer_setup(&g_flash_timer, _FlashTimer, 0);
...
void _TimerHandler(struct timer_list *t)
{
    if ((readl(lradc + 0x0c) * 3 * 3 * 10) < battery_low) {
        ...
        if (use_rumble) {
            extern void MIYOO_RUMBLE(bool);
            MIYOO_RUMBLE(true);
            timer_setup(&g_rumble_timer, _RumbleTimer, 1);
            mod_timer(&g_rumble_timer, jiffies + msecs_to_jiffies(g_rumble_interval));
        }
    }
    mod_timer(t, jiffies + msecs_to_jiffies(g_time_interval));
}
